...
uint8 GetCRC(uint8 index, uint8 hilo)
{
	uint8 crc_temp;
	uint16 temp;
	if(hilo == CRC_HI)
		temp = STMFLASH_ReadHalfWord(0x8007800 + index);
	else
		temp = STMFLASH_ReadHalfWord(0x8007900 + index);
	crc_temp = temp & 0xff;
	return crc_temp;
}
...
//	CRClo = auchCRCLo[uIndex];
	CRChi = CRClo^GetCRC(uIndex,CRC_HI);
	CRClo = GetCRC(uIndex,CRC_LO);	
}
...
//}
//uint16 CRC16(uint8 usDataLen)
u16 crc16(u8 *p, u8 length)
{
	uint16 uchCRCHi = 0xFF ; // high byte of CRC initialized 
...
//			uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex] ;
//			uchCRCLo = auchCRCLo[uIndex];
				uchCRCHi = uchCRCLo^GetCRC(uIndex,CRC_HI);
				uchCRCLo = GetCRC(uIndex,CRC_LO);	
		}
